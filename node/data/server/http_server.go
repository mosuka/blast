...
func NewHTTPServer(httpAddress string, grpcAddress string) (*HTTPServer, error) {
	return &HTTPServer{
		...
		httpAccessLogger: blastlog.DefaultLogger(),
	}, nil
}
...
func (s *HTTPServer) Start() error {
	...
	// Set HTTP handlers
	router.Handle("/", NewIndexHandler(s.logger)).Methods("GET")
	router.Handle("/metrics", promhttp.Handler()).Methods("GET")
	router.Handle("/rest/{id}", NewPutHandler(s.logger, s.grpcClient)).Methods("PUT")
	router.Handle("/rest/{id}", NewGetHandler(s.logger, s.grpcClient)).Methods("GET")
	router.Handle("/rest/{id}", NewDeleteHandler(s.logger, s.grpcClient)).Methods("DELETE")
	router.Handle("/rest/_bulk", NewBulkHandler(s.logger, s.grpcClient)).Methods("POST")
	router.Handle("/rest/_search", NewSearchHandler(s.logger, s.grpcClient)).Methods("POST")
