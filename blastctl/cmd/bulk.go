...
var bulkCmd = &cobra.Command{
	Use:   "bulk",
	Short: "indexes the documents in bulk to the Blast Server",
	Long:  `The bulk command indexes the documents in bulk to the Blast Server.`,
	RunE:  runEBulkCmd,
}
...
func runEBulkCmd(cmd *cobra.Command, args []string) error {
	...
	// create client
	cw, err := client.NewBlastClientWrapper(bulkCmdOpts.server)
	if err != nil {
		return err
	}
	defer cw.Conn.Close()
	...
	// request
	resp, err := cw.Bulk(context.Background(), requests, int32(batchSize))
	if err != nil {
		return err
	}
...
func init() {
	bulkCmd.Flags().StringVar(&bulkCmdOpts.server, "server", DefaultServer, "server to connect to")
