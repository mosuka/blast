...
func runESearchCmd(cmd *cobra.Command, args []string) error {
	...
	// create client
	cw, err := client.NewBlastClientWrapper(searchCmdOpts.server)
	if err != nil {
		return err
	}
	defer cw.Conn.Close()
	...
	// request
	resp, err := cw.Search(context.Background(), searchRequest)
	if err != nil {
		return err
	}
...
func init() {
	searchCmd.Flags().StringVar(&searchCmdOpts.server, "server", DefaultServer, "server to connect to")
